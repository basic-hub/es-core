...
if ( ! function_exists('redis_list_push')) {
    ...
    function redis_list_push(Redis $redis, string $key, $data, bool $first = false, $shardNumber = 0)
    {
        ...
        return $first ? $redis->lPush($key, $data) : $redis->rPush($key, $data);
    }
}
...
if (!function_exists('request_limit')) {
     ...
     * @throws HttpParamException
     */
    function request_limit(Redis $redis, string $cfgKey, array $input = [], $isWhite = false)
    {
        // 配置参考
        /*'REQUEST_LIMIT' => [
            'user_reg' => [
...
if (!function_exists('request_lock')) {
    /**
     ...
     * @throws HttpParamException
     */
    function request_lock(Redis $redis, string $cfgKey, array $input = [])
    {
        // 配置参考
        /*'REQUEST_LOCK' => [
            'create_order' => [
